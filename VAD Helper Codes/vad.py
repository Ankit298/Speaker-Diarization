import collections
import contextlib
import sys
import wave
import pandas as pd 
import webrtcvad
import numpy as np
from sklearn import metrics
from scipy.optimize import brentq
from scipy.interpolate import interp1d


def read_wave(path):
    """Reads a .wav file.
    Takes the path, and returns (PCM audio data, sample rate).
    """
    wf = wave.open(path,'rb')
    num_channels = wf.getnchannels()
    sample_width = wf.getsampwidth()
    sample_rate = wf.getframerate()
    no_of_samples = wf.getnframes()
    duration = no_of_samples / float(sample_rate)
    pcm_data = wf.readframes(wf.getnframes())
    return pcm_data, sample_rate,duration

def frame_generator(frame_duration_ms, audio, sample_rate,duration):
    """Generates audio frames from PCM audio data.
    Takes the desired frame duration in milliseconds, the PCM data, and
    the sample rate.
    Yields Frames of the requested duration.
    """
    ground_truth = []
    n = int(sample_rate * (frame_duration_ms / 1000.0) * 2)
    offset = 0
    timestamp = 0.0
    frame_duration = frame_duration_ms / 1000.0
    while timestamp + frame_duration < duration:
        yield audio[offset:offset + n]
        timestamp += frame_duration
        offset += n
    
def ground_truth(frame_duration_ms,frames):
    gt_df = pd.read_csv("E:/Summer2020/Local Recordings/watch_recordings/Annotations/memo21_annotated.csv")
    gt_list = []
    timestamp = 0.0
    duration = (frame_duration_ms / 1000.0)
    for frame in frames:
        for i in range(0,gt_df.shape[0]):
            j = 0
            if (timestamp > gt_df.iloc[i,0] and timestamp <= gt_df.iloc[i,1]):
                gt_list.append(1)
                j += 1
                break
        if j == 0:
            gt_list.append(0)
        # if ((timestamp >= 2.851 and timestamp <= 6.136) or (timestamp >= 6.578 and timestamp <= 13.825) or (timestamp >= 14.074 and timestamp <= 21.1) or (timestamp >= 21.376 and timestamp <= 23.171)):
        # # if ((timestamp >= 1.024 and timestamp <= 4.041) or (timestamp >= 5.192 and timestamp <= 6.064) or (timestamp >= 6.549 and timestamp <= 9.433) or (timestamp >= 10.026 and timestamp <= 12.074) or (timestamp >= 12.449 and timestamp <= 16.556) or (timestamp >= 16.787 and timestamp <= 18.556) or (timestamp >= 19.925 and timestamp <= 22.942)):
        #     gt_list.append(1)
        # else:
        #     gt_list.append(0)
        timestamp += duration
    return gt_list
    
def vad_collector(sample_rate, frame_duration_ms,vad, frames):
    pred_list = []
    for frame in frames:
        is_speech = vad.is_speech(frame, sample_rate)
        if is_speech:
            pred_list.append(1)
        else:
            pred_list.append(0)
    return pred_list
        
def main():
    audio, sample_rate, duration = read_wave('E:/Summer2020/Local Recordings/Rec1.wav')
    # audio, sample_rate, duration = read_wave('E:/Summer2020/Local Recordings/Recording1.wav')
    vad = webrtcvad.Vad(0)
    frames = frame_generator(30, audio, sample_rate,duration)
    frames = list(frames)
    pred_list = vad_collector(sample_rate, 30,vad, frames)
    gt_list = ground_truth(30, frames)
    pred_array = np.array(pred_list)
    gt_array = np.zeros((len(gt_list),))
    fpr, tpr, thresholds = metrics.roc_curve(gt_array,pred_array, pos_label=1)
    # eer = brentq(lambda x : 1. - x - interp1d(fpr, tpr)(x), 0., 1.)
    fnr = 1-tpr
    abs_diffs = np.abs(fpr - fnr)
    min_index = np.argmin(abs_diffs)
    eer = np.mean((fpr[min_index], fnr[min_index]))
    print(eer)
    print(gt_list)
    print(pred_list)

main()
